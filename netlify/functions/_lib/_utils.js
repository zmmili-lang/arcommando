...
export async function ensureSchema() {
  ...
  `
    ...
    CREATE TABLE IF NOT EXISTS leaderboard_power_history (
      id BIGSERIAL PRIMARY KEY,
      player_name TEXT NOT NULL REFERENCES leaderboard_players(name) ON DELETE CASCADE,
      power BIGINT NOT NULL,
      scraped_at BIGINT NOT NULL,
      UNIQUE(player_name, scraped_at)
    );
  `

    await sql`
    CREATE INDEX IF NOT EXISTS idx_power_history_player 
    ON leaderboard_power_history(player_name, scraped_at DESC);
  `
}
