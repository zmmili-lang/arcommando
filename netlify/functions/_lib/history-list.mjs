...
export const handler = async (event) => {
    ...
    const rows = await sql`SELECT h.ts, h.player_id, h.code, h.status, h.message, p.nickname, p.avatar_image FROM history h LEFT JOIN players p ON p.id = h.player_id WHERE h.ts >= ${start} AND h.ts < ${end}`
