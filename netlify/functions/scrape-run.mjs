...
export const handler = async (event) => {
    ...
    const { players = 110, fast = false, noApi = false, debugImages = false, savePowerAttempts = false, autoYes = true, retryLast = false, retrySession = null } = body
    ...
    try {
        ...
        // Retry logic takes precedence
        if (retryLast) {
            args.push('--retry-last')
        } else if (retrySession) {
            args.push('--retry-session', retrySession)
        } else {
            // Normal run args
